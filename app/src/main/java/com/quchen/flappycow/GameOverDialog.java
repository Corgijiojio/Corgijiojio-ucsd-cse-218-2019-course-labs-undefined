...
public class GameOverDialog extends Dialog {
    public static int REVIVE_PRICE = 5;
        ...
        okButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(game.numberOfRevive <= 1){
                    game.accomplishmentManager.saveLocal(game);
                    if(game.getApiClient().isConnected()){
                        game.accomplishmentManager.submitScore(game, game.getApiClient());
                        AccomplishmentManager.savesAreOnline(game);
                    }else{
                        AccomplishmentManager.savesAreOffline(game);
                    }
                }
    ...
    private void manageScore(){
        ...
        if(game.pointManager.point > oldPoints){
            // Save new highscore
            SharedPreferences.Editor editor = saves.edit();
            editor.putInt(best_score_key, game.pointManager.point);
            tvBestScoreVal.setTextColor(Color.RED);
            editor.commit();
        }
        tvCurrentScoreVal.setText("" + game.pointManager.point);
    ...
    private void manageMedals(){
        ...
        if(game.accomplishmentManager.achievement_gold){
            ((ImageView)findViewById(R.id.medaille)).setImageBitmap(BitmapManager.getScaledBitmapAlpha8(game, R.drawable.gold));
            if(medaille < 3){
                editor.putInt(MainActivity.medaille_key, 3);
            }
        }else if(game.accomplishmentManager.achievement_silver){
            ((ImageView)findViewById(R.id.medaille)).setImageBitmap(BitmapManager.getScaledBitmapAlpha8(game, R.drawable.silver));
            if(medaille < 2){
                editor.putInt(MainActivity.medaille_key, 2);
            }
        }else if(game.accomplishmentManager.achievement_bronze){
            ((ImageView)findViewById(R.id.medaille)).setImageBitmap(BitmapManager.getScaledBitmapAlpha8(game, R.drawable.bronce));
            if(medaille < 1){
