...
public class StartscreenView extends View{
    ...
    public StartscreenView(MainActivity context) {
        ...
        if(splash == null) {
            splash = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.splash);
        }
        srcSplash = new Rect(0, 0, splash.getWidth(), splash.getHeight());
        if(logInOut == null) {
            logInOut = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.signinout);
        }
        if(play == null) {
            play = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.play_button);
        }
        srcPlay = new Rect(0, 0, play.getWidth(), play.getHeight());
        if(achievements == null) {
            achievements = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.achievement_button);
        }
        srcAchievements = new Rect(0, 0, achievements.getWidth(), achievements.getHeight());
        if(leaderboard == null) {
            leaderboard = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.highscore_button);
        }
        srcLeaderboard = new Rect(0, 0, leaderboard.getWidth(), leaderboard.getHeight());
        if(speaker == null) {
            speaker = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.speaker);
        }
        if(info == null) {
            info = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.about);
        }
        srcInfo = new Rect(0, 0, info.getWidth(), info.getHeight());
        if(socket == null) {
            socket = BitmapManager.getBitmapAlpha8(mainActivity, R.drawable.socket);
        }
