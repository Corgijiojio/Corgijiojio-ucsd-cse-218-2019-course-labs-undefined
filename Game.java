...
//    public static SoundPool soundPool = new SoundPool(5, AudioManager.STREAM_MUSIC, 0);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        accomplishmentManager = new AccomplishmentManager();
        view = new GameView(this);
        gameOverDialog = new GameOverDialog(this);
        handler = new MyHandler(this);
        setContentView(view);
        musicManager.initMusicPlayer(this);
        loadCoins();
...
//    }
    ...
    protected void onResume() {
        view.drawOnce();
        if(musicManager.musicShouldPlay){
            musicManager.start();
        }
    ...
    public void increaseCoin(){
        this.coins++;
        if(coins >= 50 && !pointManager.achievement_50_coins){
            pointManager.achievement_50_coins = true;
    ...
    public void increasePoints(){
        ...
        this.view.getPlayer().upgradeBitmap(pointManager.points);
        
        if(pointManager.points >= PointManager.BRONZE_POINTS){
            if(!pointManager.achievement_bronze){
                pointManager.achievement_bronze = true;
                if(getApiClient().isConnected()){
            ...
            if(pointManager.points >= PointManager.SILVER_POINTS){
                if(!pointManager.achievement_silver){
                    pointManager.achievement_silver = true;
                    if(getApiClient().isConnected()){
                ...
                if(pointManager.points >= PointManager.GOLD_POINTS){
                    if(!pointManager.achievement_gold){
                        pointManager.achievement_gold = true;
                        if(getApiClient().isConnected()){
